...
static void timerHandler( int sig, siginfo_t *si, void *uc )
{
	...
	//TODO: print that the step has been completed
	comp_item->PrintComplete();
	
	//TODO: run the rempove dependancies using the signal
...
void RemoveDepHandler(int sig) {
	...
	for (int i = 0; i < completedSteps->size(); i ++) {
		currstep = completedSteps->at(i);
		recipeSteps->RemoveDependency(currstep);
	}
	/* End Section - 3 */
}
...
int main(int argc, char **argv)
{
	...
	//TODO: need a loop of some sort
	while (completedSteps->size() < recipeSteps->Count()) {
		// run get ready steps
